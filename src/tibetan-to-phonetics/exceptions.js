...
const normalize = function (exceptions) {
  return Object.keys(exceptions).reduce((hash, key) => {
    if (key.trim().length) {
      ...
      if (normalizedKey != normalizedValue) hash[normalizedKey] = exceptions[key];
    }
    return hash;
  }, {});
};
      ...
      if (exception) {
        if (modifier) {
          if (modifier.match(/(འི|ས)/)) {
            if (exception.last() == 'a') exception = exception.to(-1) + t('drengbu');
            else if (exception.last() == 'o') exception = exception.to(-1) + t('ö');
            else if (exception.last() == 'u') exception = exception.to(-1) + t('ü');
            else if (!exception.last().match(/[ieéè]/)) exception += modifier;
          } else if (modifier == 'ར') {
            if (exception.last().match(/[eéè]/)) exception = exception.to(-1) + 'er';
            else if (exception.last().match(/[aiou]/)) exception += 'r';
